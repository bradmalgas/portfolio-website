...
// Dark portfolio theme colours (mirrors lib/theme/palette.ts defaults)
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  <defs>
    ...
    <filter id="glow-tr" x="-50%" y="-50%" width="200%" height="200%">
      <feGaussianBlur stdDeviation="40"/>
    </filter>
    <filter id="glow-bl" x="-50%" y="-50%" width="200%" height="200%">
      <feGaussianBlur stdDeviation="40"/>
    </filter>
  </defs>
  ...
  <defs>
    <pattern id="dots" x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse">
      <circle cx="0.5" cy="0.5" r="0.5" fill="${accent}" fill-opacity="0.15"/>
    </pattern>
  </defs>
  <rect width="1200" height="630" fill="url(#dots)" opacity="0.08"/>

  <!-- glow top-right -->
  <circle cx="1300" cy="-100" r="250" fill="${accent}" fill-opacity="0.3" filter="url(#glow-tr)"/>

  <!-- glow bottom-left -->
  <circle cx="-50" cy="680" r="150" fill="${accent}" fill-opacity="0.18" filter="url(#glow-bl)"/>

  <!-- logo centred at 280x280 -->
  <g transform="translate(460,175) scale(0.4577,-0.4577) translate(0,-611)" fill="${accentHover}">
